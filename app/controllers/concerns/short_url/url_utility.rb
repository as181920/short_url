  ...
  module UrlUtility
    ...
    private
      ...
      def url_with_stripped_params(url, keys = [])
        URI(url).tap { |uri| uri.query = Rack::Utils.parse_nested_query(uri.query).except(*keys.map(&:to_s)).to_query.presence }.to_s
      end

      def url_by_shorten(url, keep_keys: [])
        shorten_url = ShortUrl::ShortenerService.perform(url_with_stripped_params(url, keep_keys))
        keep_params = Rack::Utils.parse_nested_query(URI(url).query).slice(*keep_keys.map(&:to_s)).compact_blank
        URI(shorten_url).tap { |uri| uri.query = keep_params.to_query.presence }.to_s
      end
  end
end
